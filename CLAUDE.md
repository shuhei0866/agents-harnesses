...
ファイルだけでなく、Issue / PR の本文・コメント・コミットメッセージも公開情報として扱う。
再現例には架空のリポジトリ名・パスを使い、技術的な条件や検証結果を保ったまま匿名化する。
個人のノート構成、端末名、内部チケット URL、他プロジェクトの運用状況やリンクを載せない。
実在する個人名・端末名を検出除外辞書に追加しない。

コミット前に author / committer のメールアドレスを確認し、個人メールを公開しない場合は
GitHub の noreply アドレスをリポジトリ単位で設定する。CI の差分検査だけでは
Issue / PR 本文やコミット情報は保護できない。

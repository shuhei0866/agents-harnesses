...
`plugins/agent-harness-flight-recorder/` に、Claude Code と Codex の lifecycle
hook を同じ privacy-first JSONL schema へ正規化する最初の共通プラグインがある。
Codex 固有の資産は引き続き `codex/` に追加する。

### Agent Harness Flight Recorder

`SessionStart` / `UserPromptSubmit` / `PostToolUse` / `Stop` をローカルに記録する。
生 prompt、code、command、tool output、transcript、workspace path は既定で保存しない。
現段階は観測のみで、自動評価・モデルルーティング・クラウド共有は非スコープ。

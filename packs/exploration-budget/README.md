...
| `want add "<問い>" --anchor <生活文脈>` / `want list` / `want close <id>` / `want feed <id> --delta <±x>` | 欲を蒔く・並べる・閉じる・出来事を手で記録する（下の「欲」）。`--project-dir` は副コマンドの後に付ける。`touch` と `checkpoint` は `--want <id>` で仕えている欲を付ける（無い・閉じた id は受けない） |
| `verdict <判定> <id> --response <回答> [--feeds <want id>]` | 朝の回答（5 値）で欲を動かす。`--feeds` を省くと、その対象に最後に付いた欲 |
...
方針は終端状態を消したが、人が探索の上に座っていたときに出していた振る舞いのうち、結果への非対称な反応、自分から差し出すこと、失望したときの切り替えは方針では出ない。欲はその置き場で、人が蒔いた「完了できない問い」を台帳に置き、いちばん飢えているものを文面の事実の直後に載せる。

- `want add "<問い>" --anchor <生活文脈> [--hunger 0.8] [--parent <id>] [--if-new]`。問いは完了できる文（N 人見つける）にしない。完了できる文はゴールに戻る。anchor は根の引き直し先になるので、時代・場・人で書く。
- エージェントは `touch --want <id>` / `checkpoint --want <id>` で、どの欲に仕えているかを付ける。満たしたかどうかは自分で申告しない（評価者と朝の回答が決める）。
- 「直近の探索した区切りは新規 0」の舵は、根の引き直し先をいちばん飢えている欲の anchor にする。新奇さそのものへ飢えさせない。
- 餌は朝に届く。`verdict <判定> <id> --response knew|interesting|delighted|low-interest|undecided [--feeds <want id>]`。回答が hunger を動かす（知っていた -0.10、知らないが面白い -0.20、関係が分かって嬉しい -0.40、関心が薄い・判断できないは 0 で記録だけ）。次の run は順位が変わった状態で始まる。
- `report` は欲ごとに仕えた量と餌の回数を並べ、「今朝の1問」を 1 件だけ出す。仮の選び方はいちばん飢えている欲に付いた最新の未判定候補（無ければ session の最新の未判定候補）。答えで動く欲は、その対象に最後に付いた欲で、`--feeds` を省いた推定と同じ。
- 判定と回答は 1 つのトランザクションで書く。欲が解決できなければ判定も書かれないので、打ち直しで二重に餌を受けない。

欲はループを回すためのもので、結果の採点には使わない。判定は人の回答で見る。この版では評価者は欲を読まない。区切りごとに「どの欲が少し満ちたか」を評価者が仮判定すること、餌が来ないまま仕え続けた欲を飢えとして起こし 幅を広げる収集（runner の外）へ引き継ぐこと、朝の回答から子の欲を産むことは未実装で、`--parent` は人が付ける。

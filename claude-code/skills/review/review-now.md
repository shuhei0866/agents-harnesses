...
**適用条件**: PR target / upstream が外部 OSS / foundation 系のリポジトリを指す場合に強制実行。

判定の優先順位:
1. `gh repo view --json parent,isFork` で fork 関係を取得 — `isFork: true` なら `parent` (上流) の owner が外部組織なら対象 (典型的な fork → upstream PR フロー)
2. `git remote get-url upstream` (設定があれば) が外部組織を指していれば対象
3. fork なしで直接 contributor になっているケースで `origin` が外部組織を指していれば対象
4. 上記いずれにも該当しない (個人 repo / internal repo) なら skip
...
1. 以下を取得:
   - `git diff HEAD` (作業中変更)
   - `git log --no-merges <base>..HEAD --pretty=fuller` (branch 上の commit messages 全件。`<base>` は PR target の対応 branch、典型的には `upstream/main` または `parent/main`)
   - 投下予定の本文 (PR description / 通知コメント等)

...
`--spec` が指定されていない場合、次の順に探し、最初に見つかったものを Reviewer 5 の入力にする。

1. diff に含まれるスペック文書。仕様と実装を同じ PR で変えているなら、それが対象である
2. リポジトリの慣習的な置き場を新しい順に探す:

   ```bash
   ls -t .claude/release-specs/*.md docs/specs/*.md specs/*.md doc/design/*.md 2>/dev/null | head -5
   ```

3. ブランチ名または直近のコミットメッセージに現れる識別子で、上の候補を絞る
4. `gh pr view --json body` が使えるなら、PR 本文が参照しているパスを拾う

**複数見つかった場合**は、diff の変更ファイルと同じ機能名を持つものを優先する。決められなければユーザーに選ばせる。**1 つも見つからなければ Reviewer 5 は起動しない**（存在しないものを探させると false positive の温床になる）。
...
- **チェック項目（実装から仕様への逆方向）**:
  ...
  - **PR 説明と実装の齟齬**: PR 本文が約束した変更のうち、diff に含まれていないものが無いか

...
Install the same sync core as a daily user-level background job:
...
macOS uses a managed `launchd` LaunchAgent with `RunAtLoad` and a daily
calendar interval. Linux uses a `systemd --user` service and timer with
`Persistent=true`. These settings catch the next login or wake after a missed
run. Install and uninstall are idempotent and refuse to overwrite an existing

...
The old seal authenticates the input generation. A v3 `full` seal starts a
...
parent seal/database/generation into an `authenticated_delta` successor. Every
delta also runs SQLite `quick_check` across all pages, preserving physical
corruption detection without repeating foreign-key and index/table cross-checks.
A legacy v1/v2 parent pays one final complete validation. Manual, policy-wide,
recovery, and unproved seal issuance still performs full validation.

...
def _read_card(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("meaning_card_id") != _sha256(
            canonical_json(
                {
                    key: nested
        ...
        or path.name
        != f"{value['meaning_card_id'].removeprefix('sha256:')}.json"
    ):
...
def _same_generation(
    ...
    return (
        ...
        and provenance.get("evaluator_adapter_sha256") == evaluator_sha256
...
def _baseline(card: dict[str, Any]) -> dict[str, Any]:
    ...
    answers = {
        name: {"state": "uncovered", "evidence_references": []}
        for name in MEANING_FIELDS
    }
    if card.get("task_type"):
        answers["intent"] = {
            "state": "covered",
            "evidence_references": evidence_ids[:1],
        }
    if card.get("changed_file_fingerprints"):
        answers["deliverable"] = {
            "state": "covered",
            "evidence_references": evidence_ids[:1],
        }
    ...
    covered_count = sum(
        item["state"] == "covered" for item in answers.values()
    )
    return {"answers": answers, "covered_count": covered_count}
...
def _comparison(
    ...
    meaning_count = 0
    for name in MEANING_FIELDS:
        meaning = meaning_card[name]
        meaning_state = (
            "covered"
            if meaning["summary"] and meaning["evidence_references"]
            else "uncovered"
        )
        meaning_count += meaning_state == "covered"
        questions[name] = {
            "baseline": baseline["answers"][name]["state"],
            "meaning": meaning_state,
        }
    baseline_count = baseline["covered_count"]
    return {
        "questions": questions,
        "baseline_covered_count": baseline_count,
        "meaning_covered_count": meaning_count,
        "coverage_lift": meaning_count - baseline_count,
    }
...
def render_generate(value: dict[str, Any]) -> str:
    ...
    return "\n".join(
        (
            ...
            (
                "Coverage: "
                f"{comparison['baseline_covered_count']}/5 -> "
                f"{comparison['meaning_covered_count']}/5 "
                f"(+{comparison['coverage_lift']})"
            ),

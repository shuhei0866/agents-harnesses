...
- Status: accepted
- Decision: issue seal contract v2 with an HMAC-protected writer-validation
  lineage. A fully validated rebuild starts the lineage. A bounded successor
  may skip vault-wide `foreign_key_check` and `integrity_check` only after
  authenticating a v2 parent and verifying, inside the write transaction, the
  ...
  binds the exact parent seal digest, database digest, and generation. A v1
  parent performs one final full validation and emits a v2 `full` seal. Manual
  rebuilds, policy-wide rewrites, recovery, and proofless seal issuance retain
  complete validation.
- Reason: after selective replay, a warm 233-event unit took 2m52s, but two
  ...
  relational validity for the managed delta; rescanning unchanged parent rows
  did not add a distinct per-delta guarantee.
- Consequence: ordinary bounded refresh cost follows the changed evidence plus
  the required streaming database digest instead of two global relational
  scans. The seal exposes whether the generation was fully or incrementally
